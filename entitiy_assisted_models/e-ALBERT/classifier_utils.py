...
class InputExample(object):
  ...
  def __init__(self, guid, text_a, entity_b, entity_a, text_b=None, label=None):
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["label_ids"] = create_float_feature([feature.label_id])\
        if task_name == "regression" else create_int_feature([feature.label_id])
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  labeltype = tf.float32 if task_name == "regression" else tf.int64
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, per_example_loss, probabilities, logits, predictions) = \
        create_model(albert_config, is_training, input_ids, input_mask,
                     ...
                     use_one_hot_embeddings, task_name, entity_a,entity_b)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      if task_name not in ["regression", "cola"]:
        ...
        def metric_fn(per_example_loss, label_ids, logits, is_real_example):
          """Compute Pearson correlations for regression."""
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
  ...
  for feature in features:
    ...
    all_entity_a.append(feature.entity_a)
    all_entity_b.append(feature.entity_b)
